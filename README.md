...
Телеграм-бот — счётчик отжиманий. Записывай подходы командой, смотри красивые
графики (в стиле Excel), соревнуйся с другими участниками в общем рейтинге и
прикрепляй фото к подходам. Все данные хранятся в SQLite — для каждого
пользователя отдельно.

## Возможности

- **Запись подходов** — `/add 20` записывает 20 отжиманий за подход.
- **Графики:**
...
- **Рейтинг** — `/top`: все участники, их суммы и процент от цели в 100 000.
- **Редактирование** — `/list` показывает номера записей, `/edit 12 25`
  исправляет, `/delete 12` удаляет (только свои записи).
...
| Команда | Описание |
|---|---|
| `/add <n>` | записать подход на `n` отжиманий |
...
| `/list` | последние записи с их номерами |
| `/edit <id> <n>` | исправить запись |
